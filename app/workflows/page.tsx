...
export default function WorkflowsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <header
        className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/[0.06]"
      ...
      <div className="relative z-10 flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full">
        ...
        <aside
          className="lg:w-64 xl:w-72 shrink-0 p-6 lg:border-r border-b lg:border-b-0 border-white/[0.06]"
        ...
        <main className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-8 space-y-6">
            ...
            {!selectedRole && (
              ...
              >
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5" style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.2), rgba(129,140,248,0.1))", border: "1px solid rgba(99,102,241,0.2)" }}>
                  <span className="text-2xl">⚡</span>
                </div>
                <h2 className="text-xl font-semibold text-white/80 mb-2">Ready to execute.</h2>
                <p className="text-sm text-white/35 max-w-xs leading-relaxed">Pick your role on the left, then describe the problem you're solving. Crazly will generate your exact AI workflow.</p>
              </div>
            )}

            {/* Role selected, awaiting input */}
            {selectedRole && !showResult && !isTyping && (
              <div className="flex items-start gap-3 max-w-2xl">
                <div className="w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold" style={{ background: "linear-gradient(135deg, #6366f1, #818cf8)" }}>
                  AI
                </div>
                <div className="flex-1">
                  <div className="px-4 py-3.5 rounded-2xl rounded-tl-sm text-sm text-white/75 leading-relaxed" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    Hey, I see you're working as a <span className="text-[#818cf8] font-medium">{roles.find(r => r.key === selectedRole)?.label}</span>. Tell me what you're stuck on — be specific. I'll generate an exact AI workflow and toolset for your situation.
                  </div>
            ...
            {(isTyping || showResult) && input && (
              <div className="flex items-start gap-3 max-w-2xl ml-auto flex-row-reverse">
                <div className="w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-semibold bg-white/10 text-white/60">
                  You
                </div>
                <div className="flex-1 flex flex-col items-end">
                  <div className="px-4 py-3.5 rounded-2xl rounded-tr-sm text-sm text-white/85 leading-relaxed max-w-sm" style={{ background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.25)" }}>
                    {input}
                  </div>
            ...
            {showResult && selectedRole && (
              <div className="flex items-start gap-3 max-w-2xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Title bubble */}
                  <div className="px-4 py-3.5 rounded-2xl rounded-tl-sm mb-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  {/* Tools bubble */}
                  <div className="px-4 py-3.5 rounded-2xl mb-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div className="px-4 py-3.5 rounded-2xl mb-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    ...
                    <div className="space-y-3">
                      {workflows[selectedRole].steps.map((step, i) => (
                        ...
                        >
                          <div className="w-5 h-5 rounded-full shrink-0 flex items-center justify-center text-[10px] font-bold mt-0.5" style={{ background: "rgba(99,102,241,0.2)", color: "#818cf8", border: "1px solid rgba(99,102,241,0.3)" }}>
                            {i + 1}
                          </div>
                  ...
                  >
                    <div className="px-4 py-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-sm" style={{ background: "rgba(251,191,36,0.1)" }}>
                          🔒
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-amber-300">Unlock Premium Prompts</p>
                          <p className="text-[11px] text-white/35">Get exact AI prompts, scripts & pro templates for this workflow</p>
                        </div>
                      </div>
                      <button className="shrink-0 px-4 py-2 rounded-xl text-xs font-semibold text-amber-900 transition-all duration-200 hover:opacity-90 active:scale-95" style={{ background: "linear-gradient(135deg, #fbbf24, #f59e0b)" }}>
                        Upgrade
                      </button>
                    </div>
                  </div>
          ...
          {selectedRole && (
            <div
              className="px-4 sm:px-8 py-5 border-t border-white/[0.06]"
            ...
            >
              ...
              >
                <input
                  ...
                  disabled={isTyping}
                  className="flex-1 bg-transparent text-sm text-white/80 placeholder:text-white/25 outline-none disabled:opacity-50"
                />
                <button
              ...
              <p className="text-[10px] text-white/20 text-center mt-2.5">Press Enter to generate · Crazly simulates AI workflows</p>
            </div>
          )}
        </main>
      </div>
