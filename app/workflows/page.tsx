...
/* ─── EMAIL MODAL (only shown to non-signed-in users) ─────────── */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-md rounded-2xl p-6 sm:p-8"
        ...
        <button onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-xl flex items-center justify-center text-white/30 hover:text-white/60 hover:bg-white/5 transition-all">
          ✕
        </button>
...
export default function WorkflowsPage() {
  ...
  // Stream workflow steps one by one
  useEffect(() => {
    if (showResult && selectedRole) {
      ...
      const iv = setInterval(() => {
        ...
        if (count >= total) {
          clearInterval(iv);
          // Only show email popup for non-signed-in users, and only once
  ...
  // Fire the workflow after 1.8s typing delay
  ...
  // Handle send — gate 2nd prompt behind Clerk sign in
  const handleGenerate = () => {
    if (!input.trim() || !selectedRole) return;
    if (promptCount >= 1 && !isSignedIn) {
      // Send them to Clerk sign in page
  ...
  // After first prompt, non-signed-in users are locked
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col"
      ...
      {/* Email modal — only for non-signed-in users */}
      ...
      <header className="relative z-10 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.06]"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* Auth — real Clerk */}
          {isSignedIn ? (
            // Signed in → show Clerk avatar with dropdown (sign out etc)
            <UserButton afterSignOutUrl="/" />
          ) : (
            // Not signed in → show Sign in button that opens Clerk modal
      ...
      <div className="relative z-10 flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full">
        ...
        <aside className="hidden lg:flex lg:w-64 xl:w-72 shrink-0 flex-col p-6 border-r border-white/[0.06]"
          ...
          <div className="mt-8 p-4 rounded-xl"
            ...
            <p className="text-[11px] text-white/35 mb-3 leading-relaxed">
              Unlock exact prompts, scripts & templates for every workflow.
            </p>
            <Link href="/pricing"
        ...
        <main className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 sm:py-8 space-y-5 sm:space-y-6">
            ...
            {!selectedRole && (
              <div className="flex flex-col items-center justify-center h-full min-h-[360px] text-center px-4"
                ...
                <p className="text-sm text-white/35 max-w-xs leading-relaxed hidden lg:block">
                  Pick your role on the left, then describe the problem you're solving.
                </p>
            ...
            {/* AI greeting after role selected */}
                ...
                <div className="flex-1">
                  <div className="px-3.5 sm:px-4 py-3 sm:py-3.5 rounded-2xl rounded-tl-sm text-sm text-white/75 leading-relaxed"
                    ...
                    {isSignedIn && user?.firstName ? (
                      <>Hey <span className="text-[#818cf8] font-medium">{user.firstName}</span>! I see you're a <span className="text-[#818cf8] font-medium">{roles.find(r => r.key === selectedRole)?.label}</span>. Tell me what you're stuck on — I'll generate your exact AI workflow.</>
                    ) : (
                  ...
                  <div className="rounded-2xl overflow-hidden"
                    ...
                    <div className="px-3.5 sm:px-4 py-3.5 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        {isSignedIn ? (
                          // Signed-in users go straight to pricing
                          <Link href="/pricing"
                          ...
                          // Non-signed-in users see email capture
                          <button
          ...
          {selectedRole && (
            <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-white/[0.06]"
              ...
              {/* Sign in gate — appears after 1st prompt for non-signed-in users */}
              {inputLocked && (
              ...
              <div className="flex items-center gap-2.5 sm:gap-3 px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl transition-all"
                style={{
                  ...
                  border: `1px solid ${inputLocked ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.09)"}`,
                }}>
                <input
                  ref={inputRef}
                  placeholder={
                    inputLocked
                      ? "Sign in to send another prompt..."
                      : `Describe your ${roles.find(r => r.key === selectedRole)?.label.toLowerCase()} problem...`
                  }
                  value={input}
              ...
              <p className="text-[10px] text-white/20 text-center mt-2">
                {inputLocked
                  ? "Free account required for more prompts"
