...
const TOOL_URLS: Record<string, string> = {
  ...
  "perplexity ai": "https://perplexity.ai", "notebooklm": "https://notebooklm.google.com",
  "v0": "https://v0.dev", "v0 by vercel": "https://v0.dev",
  "github copilot": "https://github.com/features/copilot",
  "notion ai": "https://notion.so", "notion": "https://notion.so",
  "grammarly": "https://grammarly.com", "hemingway": "https://hemingwayapp.com",
  "hemingway editor": "https://hemingwayapp.com", "midjourney": "https://midjourney.com",
  "canva": "https://canva.com", "canva ai": "https://canva.com",
  "figma": "https://figma.com", "linear": "https://linear.app",
  "supabase": "https://supabase.com", "vercel": "https://vercel.com",
  "typeform": "https://typeform.com", "loom": "https://loom.com",
  "descript": "https://descript.com", "opus clip": "https://opus.pro",
  "opus": "https://opus.pro", "buffer": "https://buffer.com",
  "metricool": "https://metricool.com", "apollo": "https://apollo.io",
  "apollo.io": "https://apollo.io", "lemlist": "https://lemlist.com",
  "anki": "https://ankiweb.net", "quizlet": "https://quizlet.com",
  "consensus": "https://consensus.app", "elicit": "https://elicit.org",
  "zotero": "https://zotero.org", "wolfram alpha": "https://wolframalpha.com",
  "desmos": "https://desmos.com", "semrush": "https://semrush.com",
  "vidiq": "https://vidiq.com", "sparktoro": "https://sparktoro.com",
  "hotjar": "https://hotjar.com", "looker studio": "https://lookerstudio.google.com",
  "klaviyo": "https://klaviyo.com", "postman": "https://postman.com",
  "sentry": "https://sentry.io", "sonarqube": "https://sonarqube.org",
  "beautiful.ai": "https://beautiful.ai", "tome": "https://tome.app",
  "docsend": "https://docsend.com", "contra": "https://contra.com",
  "adcreative.ai": "https://adcreative.ai", "make": "https://make.com",
  "zapier": "https://zapier.com", "n8n": "https://n8n.io",
};
...
export default function WorkflowsPage() {
  ...
  // Usage — ref for async safety, state for UI
  ...
  useEffect(() => {
    ...
    const syncUser = async () => {
      ...
      try {
        ...
        // Get their usage + plan
        const res = await fetch("/api/user");
        const data = await res.json();
        const plan = data.plan ?? "free";
        const count = data.message_count ?? 0;
        setUserPlan(plan);
        usageCountRef.current = plan === "pro" ? 0 : count;
        setUsageDisplay(plan === "pro" ? 0 : count);
        // If they already hit the limit, show paywall
  ...
  const effectiveLimit = userPlan === "pro" ? Infinity : FREE_LIMIT;
  ...
  const handleRoleSelect = (role: Role) => {
    ...
    setStreamingText("");
    setInput("");
    // Don't reset usage count when switching roles — it's per-session/account
    setShowPaywall(usageCountRef.current >= effectiveLimit);
    setDrawerOpen(false);
  ...
  const handleSend = async () => {
    ...
    if (usageCountRef.current >= effectiveLimit) {
      setShowPaywall(true);
      return;
    }
    ...
    // Increment count — ref is synchronous (never stale)
    usageCountRef.current += 1;
    setUsageDisplay(usageCountRef.current);

    // Save to Supabase if signed in
    if (isSignedIn) {
      fetch("/api/user", { method: "PATCH" }).catch(console.error);
    }
    ...
    try {
      ...
      // Save workflow to Supabase if signed in
      if (isSignedIn) {
        const parsed = parseWorkflow(full);
  ...
  const renderAssistantMessage = (content: string, isLive = false, msgIndex = 0) => {
    ...
    const remaining = userPlan === "pro" ? Infinity : Math.max(FREE_LIMIT - usageDisplay, 0);
          ...
          <div className="flex-1 min-w-0 space-y-2.5">
                  ...
                  {parsed.steps.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <a href={step.toolUrl} target="_blank" rel="noopener noreferrer" className="text-[11px] text-[#6366f1] hover:text-[#818cf8] transition-colors flex items-center gap-1">Open {step.tool} ↗</a>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {userPlan !== "pro" && (
              <p className="text-[10px] text-white/20 ml-1">
                Crazly AI · {remaining === Infinity ? "∞" : remaining} free message{remaining === 1 ? "" : "s"} remaining
              </p>
            )}
          </div>
        </div>
    ...
    <div className="mx-4 sm:mx-8 my-4 rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.15),rgba(129,140,248,0.08))", border: "1px solid rgba(99,102,241,0.4)" }}>
      <div className="p-6 text-center">
        ...
        <p className="text-sm text-white/50 mb-5 max-w-sm mx-auto leading-relaxed">
          Upgrade to Pro for unlimited workflows, full conversation memory, and priority AI.
        </p>
    ...
    <div className="min-h-screen bg-[#080808] text-white flex flex-col" style={{ fontFamily: "'DM Sans',system-ui,sans-serif" }}>
      ...
      <header className="relative z-10 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.06]"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {isSignedIn ? <UserButton afterSignOutUrl="/" /> : (
            <SignInButton mode="modal">
              <button className="text-xs px-3 py-1.5 rounded-xl font-medium text-white/60 hover:text-white/90 transition-all" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>Sign in</button>
            </SignInButton>
          )}
          <div className="hidden sm:flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs text-white/40">{loadingUser ? "Syncing…" : "AI Online"}</span>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full">
        ...
        <aside className="hidden lg:flex lg:w-64 xl:w-72 shrink-0 flex-col p-6 border-r border-white/[0.06]"
          ...
          {/* Pro badge */}
          {userPlan === "pro" && (
            <div className="mt-6 p-3.5 rounded-xl" style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)" }}>
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg,#6366f1,#818cf8)" }}>
              ...
              <p className="text-[11px] text-white/40 mt-1">Unlimited workflows</p>
            </div>
          )}
        ...
        <main className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 sm:py-8 space-y-5 sm:space-y-6">
            {!selectedRole && (
              <div className="flex flex-col items-center justify-center h-full min-h-[360px] text-center px-4" style={{ opacity: animateIn ? 1 : 0, transition: "opacity 0.6s ease 0.2s" }}>
                ...
                <p className="text-sm text-white/35 max-w-xs leading-relaxed hidden lg:block">Pick your role, describe your problem, get a real AI workflow with exact prompts and tool links.</p>
                ...
                <div className="flex-1">
                  <div className="px-4 py-3.5 rounded-2xl rounded-tl-sm text-sm text-white/75 leading-relaxed" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    {isSignedIn && user?.firstName
                      ? <>Hey <span className="text-[#818cf8] font-medium">{user.firstName}</span>! You selected <span className="text-[#818cf8] font-medium">{roles.find(r => r.key === selectedRole)?.label}</span>. Describe your exact problem and I'll build a personalised workflow with tool links and ready-to-paste prompts.</>
                      : <>Hey! You selected <span className="text-[#818cf8] font-medium">{roles.find(r => r.key === selectedRole)?.label}</span>. Describe your problem and I'll generate a step-by-step workflow. <span className="text-white/40">{userPlan === "pro" ? "Unlimited workflows on Pro ✨" : "3 free messages."}</span></>
                    }
                  </div>
          ...
          {selectedRole && !showPaywall && (
            <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-white/[0.06]" style={{ background: "rgba(8,8,8,0.9)", backdropFilter: "blur(12px)" }}>
              <div className="flex items-center gap-2.5 px-4 py-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)" }}>
                <input ref={inputRef}
                  placeholder={messages.length > 0 ? "Ask a follow-up or describe another problem…" : `Describe your ${roles.find(r => r.key === selectedRole)?.label.toLowerCase()} problem…`}
              ...
              <p className="text-[10px] text-white/20 text-center mt-2">
                {userPlan === "pro"
                  ? "Pro · Unlimited workflows · Saves to your history"
                  : `${Math.max(FREE_LIMIT - usageDisplay, 0)} free message${FREE_LIMIT - usageDisplay === 1 ? "" : "s"} remaining · Real AI`}
              </p>
            </div>
          )}
          ...
          {selectedRole && showPaywall && (
            <div className="px-4 sm:px-8 py-4 border-t border-white/[0.06] flex items-center justify-center gap-3" style={{ background: "rgba(8,8,8,0.9)" }}>
              ...
              {!isSignedIn && (
                <SignInButton mode="modal">
                  <button className="text-xs text-white/40 hover:text-white/70 transition-colors">Sign in to check your plan</button>
                </SignInButton>
              )}
            </div>
          )}
