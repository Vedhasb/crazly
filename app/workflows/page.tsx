...
export default function WorkflowsPage() {
  ...
  useEffect(() => { setTimeout(() => bottomRef.current?.scrollIntoView({ behavior:"smooth", block:"end" }), 80); }, [messages, streamingText]);
  ...
  const handleCopy = (text: string, key: string) => {
    navigator.clipboard.writeText(text); setCopiedIndex(key);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("/api/generate-workflow", {
        ...
        body: JSON.stringify({ role: selectedRole, messages: newMessages.map(m => ({ role: m.role, content: m.content })) }),
      });
          ...
          <div className="flex-1 min-w-0 space-y-2.5">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)" }}>
              <div className="px-4 pt-3.5 pb-1">
                ...
                <p className="text-[11px] text-white/30 mb-3">Each step has the tool link + ready-to-use prompt. Click the tool name → it opens → paste the prompt.</p>
              </div>
            ...
            <p className="text-[10px] text-white/20 ml-1">Crazly AI · {FREE_LIMIT - usageCount >= 0 ? FREE_LIMIT - usageCount : 0} free message{FREE_LIMIT - usageCount === 1 ? "" : "s"} remaining</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col" style={{ fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full">
        <aside className="hidden lg:flex lg:w-64 xl:w-72 shrink-0 flex-col p-6 border-r border-white/[0.06]" style={{ opacity:animateIn?1:0, transform:animateIn?"none":"translateX(-12px)", transition:"opacity 0.5s ease 0.1s,transform 0.5s ease 0.1s" }}>
          ...
          {selectedRole && (
            <div className="mt-6 p-3.5 rounded-xl" style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)" }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-[11px] font-bold" style={{ color:usageCount>=FREE_LIMIT?"#f87171":"#818cf8" }}>{usageCount}/{FREE_LIMIT}</p>
              </div>
              <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div className="h-full rounded-full transition-all duration-500" style={{ width:`${Math.min((usageCount/FREE_LIMIT)*100,100)}%`, background:usageCount>=FREE_LIMIT?"linear-gradient(90deg,#f87171,#ef4444)":"linear-gradient(90deg,#6366f1,#818cf8)" }} />
              </div>
              {usageCount>=FREE_LIMIT
                ...
                : <p className="text-[10px] text-white/25 mt-1.5">{FREE_LIMIT-usageCount} remaining · then upgrade</p>
              }
            </div>
          )}
        ...
        <main className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1">
                  <div className="px-4 py-3.5 rounded-2xl rounded-tl-sm text-sm text-white/75 leading-relaxed" style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)" }}>
                    {isSignedIn && user?.firstName
                      ? <>Hey <span className="text-[#818cf8] font-medium">{user.firstName}</span>! You selected <span className="text-[#818cf8] font-medium">{roles.find(r=>r.key===selectedRole)?.label}</span>. Describe your exact problem — I'll build a personalised workflow with tool links and ready-to-paste prompts for each step.</>
                      : <>Hey! You selected <span className="text-[#818cf8] font-medium">{roles.find(r=>r.key===selectedRole)?.label}</span>. Describe your problem and I'll generate a step-by-step workflow — every prompt is ready to paste and every tool has a direct link. <span className="text-white/40">3 free messages.</span></>
                    }
                  </div>
          ...
          {selectedRole && !showPaywall && (
            <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-white/[0.06]" style={{ background:"rgba(8,8,8,0.9)", backdropFilter:"blur(12px)" }}>
              ...
              <p className="text-[10px] text-white/20 text-center mt-2">
                {usageCount<FREE_LIMIT ? `${FREE_LIMIT-usageCount} free message${FREE_LIMIT-usageCount===1?"":"s"} remaining · Real AI · Remembers your conversation` : "Upgrade to continue"}
              </p>
            </div>
          )}
          ...
          {selectedRole && showPaywall && (
            <div className="px-4 sm:px-8 py-4 border-t border-white/[0.06] flex items-center justify-center gap-3" style={{ background:"rgba(8,8,8,0.9)" }}>
              ...
              <button onClick={() => { setShowPaywall(false); setUsageCount(0); setMessages([]); }} className="text-xs text-white/30 hover:text-white/60 transition-colors">Start over (resets count)</button>
            </div>
          )}
        </main>
      </div>
