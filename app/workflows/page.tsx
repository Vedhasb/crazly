...
function AuthModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    setLoading(true);
    // Simulated — swap with Clerk / Supabase later
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-sm rounded-2xl p-6 sm:p-8"
        style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
        <button onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-xl flex items-center justify-center text-white/30 hover:text-white/60 hover:bg-white/5 transition-all">✕</button>
        ...
        <p className="text-sm text-white/40 mb-6">
          {mode === "signup" ? "Sign up free to generate unlimited workflows." : "Log in to continue where you left off."}
        </p>
        <div className="flex flex-col gap-3 mb-4">
          <input type="email" placeholder="Email address" value={email}
            onChange={e => setEmail(e.target.value)}
            className="w-full px-4 py-3 rounded-xl text-sm text-white bg-white/5 border border-white/10 outline-none focus:border-[#6366f1]/50 placeholder:text-white/25" />
        ...
        <button onClick={handleSubmit}
          ...
          className="w-full py-3 rounded-xl text-sm font-semibold transition-all active:scale-[0.98] disabled:opacity-40 mb-4"
          style={{ background: "linear-gradient(135deg, #6366f1, #818cf8)" }}>
          {loading ? "Signing in..." : mode === "signup" ? "Create free account" : "Log in"}
        </button>
        <p className="text-xs text-white/30 text-center">
          {mode === "signup" ? "Already have an account? " : "New to Crazly? "}
...
export default function WorkflowsPage() {
    ...
    if (showResult && selectedRole) {
      ...
      const iv = setInterval(() => {
        ...
        if (count >= total) {
          clearInterval(iv);
          if (!emailShown) {
            setTimeout(() => { setEmailShown(true); setEmailSource("workflow"); setShowEmail(true); }, 900);
          }
        }
      }, 280);
  ...
  useEffect(() => {
    if (showResult) setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "end" }), 100);
  }, [showResult, visibleSteps]);

  const fireWorkflow = (text: string) => {
    setShowResult(false); setIsTyping(true);
  ...
  const handleGenerate = () => {
    ...
    if (promptCount >= 1 && !isAuthed) { setPendingInput(input); setShowAuth(true); return; }
    fireWorkflow(input);
  };

  const handleAuthSuccess = () => {
    setIsAuthed(true); setShowAuth(false);
    if (pendingInput) { setInput(pendingInput); setPendingInput(""); fireWorkflow(pendingInput); }
  };

  const handleRoleSelect = (role: Role) => {
    setSelectedRole(role); setShowResult(false); setIsTyping(false);
    setInput(""); setDrawerOpen(false);
    setTimeout(() => inputRef.current?.focus(), 100);
  };

  const inputLocked = promptCount >= 1 && !isAuthed;
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col"
      ...
      {showEmail && <EmailModal onClose={() => setShowEmail(false)} source={emailSource} />}
      ...
      <header className="relative z-10 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.06]"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {isAuthed ? (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-medium text-white/60"
            ...
            <button onClick={() => setShowAuth(true)}
              className="text-xs px-3 py-1.5 rounded-xl font-medium text-white/60 hover:text-white/90 transition-all"
      ...
      <div className="relative z-10 flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full">
        ...
        {/* Sidebar */}
        <aside className="hidden lg:block lg:w-64 xl:w-72 shrink-0 p-6 border-r border-white/[0.06]"
        ...
        <main className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 sm:py-8 space-y-5 sm:space-y-6">
            ...
            {!selectedRole && (
              <div className="flex flex-col items-center justify-center h-full min-h-[360px] text-center px-4"
                ...
                <button className="lg:hidden mt-5 px-5 py-2.5 rounded-xl text-sm font-semibold active:scale-95"
                  style={{ background: "linear-gradient(135deg, #6366f1, #818cf8)" }}
              ...
              <div className="flex items-start gap-2.5 sm:gap-3 max-w-2xl">
                ...
                <div className="flex-1">
                  <div className="px-3.5 sm:px-4 py-3 sm:py-3.5 rounded-2xl rounded-tl-sm text-sm text-white/75 leading-relaxed"
                    ...
                    Hey, I see you're working as a <span className="text-[#818cf8] font-medium">{roles.find(r => r.key === selectedRole)?.label}</span>. Tell me what you're stuck on — be specific. I'll generate an exact AI workflow and toolset.
                  </div>
              ...
              <div className="flex items-start gap-2.5 sm:gap-3 max-w-2xl ml-auto flex-row-reverse">
                ...
                <div className="flex-1 flex flex-col items-end">
                  <div className="px-3.5 sm:px-4 py-3 sm:py-3.5 rounded-2xl rounded-tr-sm text-sm text-white/85 leading-relaxed max-w-[85%] sm:max-w-sm"
                    style={{ background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.25)" }}>{input}</div>
                  ...
                  <div className="rounded-2xl overflow-hidden"
                    ...
                    <div className="px-3.5 sm:px-4 py-3.5 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        <button
                          onClick={() => { setEmailSource("upgrade"); setShowEmail(true); }}
          ...
          {selectedRole && (
            <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-white/[0.06]"
              ...
              {inputLocked && (
                <div className="mb-3 flex items-center justify-between px-4 py-3 rounded-2xl"
                  style={{ background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.25)" }}>
                  <div className="flex items-center gap-2.5">
                    <span className="text-base">🔐</span>
                    <div>
                      <p className="text-xs font-semibold text-white/80">Sign in to continue</p>
                      <p className="text-[11px] text-white/35">Free account — generate unlimited workflows</p>
                    </div>
                  </div>
                  <button onClick={() => setShowAuth(true)}
                    className="shrink-0 px-4 py-2 rounded-xl text-xs font-semibold hover:opacity-90 active:scale-95 transition-all"
                    style={{ background: "linear-gradient(135deg, #6366f1, #818cf8)" }}>
                    Sign up free
                  </button>
                </div>
              ...
              <div className="flex items-center gap-2.5 sm:gap-3 px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl transition-all"
                style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${inputLocked ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.09)"}` }}>
                <input ref={inputRef}
                  placeholder={inputLocked ? "Sign in to send another prompt..." : `Describe your ${roles.find(r => r.key === selectedRole)?.label.toLowerCase()} problem...`}
                  value={input}
              ...
              <p className="text-[10px] text-white/20 text-center mt-2">
                {inputLocked ? "Free account required for more prompts" : "Press Enter to generate · Crazly simulates AI workflows"}
              </p>
            </div>
          )}
        </main>
